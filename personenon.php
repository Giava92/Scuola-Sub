...
<html>
    ...
    <body>
        ...
        <div id="breadcrumb">
            ...
            <a href="sceglipersone.php"> Ricerca persone</a> >> Ne' soci ne' sub
        </div>
        ...
        <div id="content">
            ...
            echo "<h1>Persone che non sono ne' soci ne' sub</h1>";
            require_once ("con_db.php");
            $conn = connect();
            $query = "SELECT p.* FROM (Persone p LEFT JOIN Soci s ON p.idpersona=s.idsocio) 
            LEFT JOIN Sub su ON p.idpersona=su.idsub WHERE s.idsocio IS NULL AND su.idsub IS NULL";
            $result = mysql_query($query, $conn) or die("Query fallita" . mysql_error($conn));
            require_once ("fun_tabelle.php");
            table_start(array("Id persona", "Nome", "Cognome", "Data di nascita", "Email", "Telefono"));
