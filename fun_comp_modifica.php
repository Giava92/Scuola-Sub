...
function comp_mod($cosa, $quale, $quanto) {
    $conn = connect();
    switch ($cosa) {
        case 'AttrezzatureScuola':
            if (!isset($_SESSION['codatt']) && !isset($_SESSION['tipo']) && !isset($_SESSION['prezzonoleggio']))
                ...
                $query = "UPDATE $cosa SET $quale =\"" . $quanto . "\"";
                $where = " WHERE TRUE";
                if ($codatt)
                ...
                $result = mysql_query($query, $conn) or die("<p>Query fallita. " . mysql_error($conn) .
                                "</p><p>Prova a rieffettuare l'<a href=\"sceglimodifica.php\">operazione</a>.</p>");
        ...
        case 'Brevetti':
            if (!isset($_SESSION['brevetto']) && !isset($_SESSION['nome']) && !isset($_SESSION['maxprofondita']) && !isset($_SESSION['sub']))
                ...
                table_start(array("Brevetto", "Nome", "Profondita' max", "Sub"));
                while ($row = mysql_fetch_row($result1))
                    table_row($row);
                table_end();
            }
