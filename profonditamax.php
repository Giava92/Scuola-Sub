...
<html>
    ...
    <body>
        ...
        <div id="breadcrumb">
            ...
            <a href="sceglipersone.php"> Ricerca persone</a> >><a href="cerca_profonditamax.php"> Scelta sub</a> >> Profondita' massima sub
        </div>
        ...
        <div id="content">
            ...
            if ($username != "Admin" && $username != "admin") {
                ...
                echo "<p>Non hai scelto l'id del sub. <a href=\"cerca_profonditamax.php\">Riprova</a>.</p>";
            } else {
                $sub = $_POST['sub'];
                require_once ("con_db.php");
                $conn = connect();
                $query = "SELECT * FROM Sub WHERE idsub = $sub";
                $result = mysql_query($query, $conn) or die("<p>Query fallita. " . mysql_error($conn) . ".</p>");
                $row = mysql_num_rows($result);
                if ($row == '0')
                    echo "<p>Non ci sono sub con l'id selezionato.</p><p><a href=\"cerca_profonditamax.php\">Riprova</a>.</p>";
                else {
                    $query1 = "SELECT max_prof($sub)";
                    $result1 = mysql_query($query1, $conn) or die("<p>Query fallita. " . mysql_error($conn) . ".</p>");
                    $row = mysql_fetch_row($result1);
                    echo "<p>La massima profondita' a cui puo' andare il sub con id " . $sub . " e': " . $row[0] . " metri.</p>"
                    . "<p>Torna <a href=\"cerca_profonditamax.php\">indietro</a>.</p>"
