    ...
    switch ($cosa) {
        case 'AttrezzatureScuola':
            if (!isset($_POST['codatt']) && !isset($_POST['tipo']) && !isset($_POST['prezzonoleggio']))
                ...
                if (empty($num_row))
                    ...
                    <p>Prova a scegliere di nuovo i dati da eliminare  <a href=\"sceglicancella.php\">qui</a>.</p>";
                else {
                    require_once ("fun_tabelle.php");
                    echo "<p>I dati che si vogliono eliminare sono:</p>";
                    table_start(array("Codatt", "Tipo", "Prezzonoleggio"));
                    while ($row = mysql_fetch_row($result))
                        table_row($row);
                    table_end();
                    echo "<p>Per completare l'eliminazione e vedere la tabella aggiornata clicca <a href=\"comp_cancella.php\">qui</a>.</p>
            </p><p>Torna <a href=\"sceglicancella.php\">indietro</a>.</p><p>Oppure torna alla <a href=\"index.php\">home</a>.</p>";
                }
                break;
            }
        case 'Brevetti':
        ...
        case 'Eventi':
            if (!isset($_POST['idevento']) && !isset($_POST['nome']) && !isset($_POST['data']) && !isset($_POST['luogo']))
                ...
                $num_row = mysql_num_rows($result);
                if (empty($num_row))
                    echo "<p>Nessun campo con queste caratteristiche e' presente nella tabella selezionata</br></p>
                    <p>Prova a scegliere di nuovo i dati da eliminare  <a href=\"sceglicancella.php\">qui</a>.</p>";
                else {
                    require_once ("fun_tabelle.php");
                    echo "<p>I dati che si vogliono eliminare sono:</p>";
        ...
        case 'Immersioni':
            if (!isset($_POST['codimm']) && !isset($_POST['data']) && !isset($_POST['ora']) && !isset($_POST['citta']) && !isset($_POST['luogo']) && !isset($_POST['tipo']) && !isset($_POST['inizioimm']) && !isset($_POST['costo']) && !isset($_POST['profonditamax']))
                ...
                if (empty($num_row))
                    ...
                    require_once ("fun_tabelle.php");
                    echo "<p>I dati che si vogliono eliminare sono:</p>";
                    table_start(array("Codimm", "Data", "Ora", "Città", "Luogo", "Tipo", "Inizioimm", "Costo"));
                    while ($row = mysql_fetch_row($result))
                        table_row($row);
                    table_end();
                    echo "<p>Per completare l'eliminazione e vedere la tabella aggiornata clicca <a href=\"comp_cancella.php\">qui</a>.</p>
